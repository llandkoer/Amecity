...
const takeChallenge = async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).json({ message: "This user does not exist on our database" });

    const challenge = getConnection().get("challenges").find({ challenge_id }).value();
    if (!challenge) return res.status(400).json({ message: "This challenge does not exist on our database" });

    const isMyChallenge = getConnection().get("detail_challenges").find({ challenge_id }).value();
...
const achieveChallenge = async (req, res) => {
  try {
    ...
    const challenge = await getConnection().get("challenges").find({ challenge_id }).value();
    if (!challenge) return res.status(400).json({ message: "This challenge does not exist on our database" });

    const isMyChallenge = await getConnection().get("detail_challenges").find({ challenge_id }).value();
    if (!isMyChallenge) return res.status(400).send({ message: "Challenge hasn't been taken" });
    if (isMyChallenge.status !== "doing") return res.status(400).send({ message: "Challenge is already done" });

    await getConnection().get("detail_challenges").find({ challenge_id }).assign({ status: "done" }).write();
...
const getAchieved = async (req, res) => {
  try {
    ...
    const detailChallenges = await getConnection().get("detail_challenges").value();
    ...
    if (achievedChallenges.length === 0) return res.status(400).json({ message: "No achieved challenges to show" });
...
const getCurrent = async (req, res) => {
  try {
    ...
    if (currentChallenges.length === 0) return res.status(400).json({ message: "No challenges in progress to show" });
