...
const takeChallenge = async (req, res) => {
  try {
    ...
    if (!challenge)
      return res
        .status(400)
        .json({ message: "This challenge does not exist on our database" });
...
const getAchieved = async (req, res) => {
  try {
    ...
    const detailChallenges = await getConnection()
      .get("detail_challenges")
      .value();
    ...
    if (achievedChallenges.length === 0)
      return res
        .status(400)
        .json({ message: "No achieved challenges to show" });
...
const getCurrent = async (req, res) => {
  try {
    ...
    if (currentChallenges.length === 0)
      return res
        .status(400)
        .json({ message: "No challenges in progress to show" });
