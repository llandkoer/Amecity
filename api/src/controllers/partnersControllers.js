  ...
  try {
    ...
    if (!Array.isArray(rewards)) {
      return res.status(400).json(
          {message : "Rewards must exist and be an array"});
    }
    ...
    for (let i = 0; i < rewards.length; i++) {
      ...
      if (typeof element !== "string") {
        return res.status(400).json(
            {message : `Rewards on index ${i} is not a string`});
      }
    }
    ...
    if (partner) {
      return res.status(409).json(
          {message : "Your partner already exist on our database"});
    }
...
const updateRewards = async (req, res) => {
  try {
    ...
    if (!partner) {
      return res.status(409).json(
          {message : "Your partner does not exist on our database"});
    }
    ...
    res.status(200).json(
        {message : "Partners rewards have been updated successfully"});
  } catch (error) {
    res.status(500).json({error, message : "There was a server error"});
  }
};
